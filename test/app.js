...
function showFirstHalfReviewModal() {
    ...
    reviewContainer.innerHTML = `
        <div class="bg-gradient-to-br from-white to-gray-50 rounded-xl p-5 border-2 ${team.position <= 3 ? 'border-indigo-300' : 'border-gray-200'}">
            ...
            <p class="text-sm text-gray-700 mb-4 bg-indigo-50 p-3 rounded-lg">
                <span id="review-typed-text"></span>
            </p>
    ...
    // Start typing the review text
    typeReviewText(team.review, 'review-typed-text', 25);
    ...
    if (nextBtn) nextBtn.disabled = currentReviewIndex === teamReviewsData.length - 1;
    if (counter) counter.innerText = `${currentReviewIndex + 1} / ${teamReviewsData.length}`;
}
...
// ==================== TYPEWRITER FOR REVIEWS ====================
let reviewTypewriterTimer = null;

function typeReviewText(text, elementId, speed = 25) {
    const el = document.getElementById(elementId);
    if (!el) return;
    // Stop any previous typewriter
    if (reviewTypewriterTimer) {
        clearTimeout(reviewTypewriterTimer);
        reviewTypewriterTimer = null;
    }
    el.textContent = '';
    let index = 0;
    function typeNext() {
        if (index < text.length) {
            ...
            index++;
            reviewTypewriterTimer = setTimeout(typeNext, speed);
        } else {
