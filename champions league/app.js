...
const CURRENT_LEAGUE = 'championsleague';
...
function saveToStorage() {
    const data = {
        ...
        players: teams,
        qualificationPlayoffs: tournament.qualificationPlayoffs,
...
function generateQualificationPlayoff(roundIndex) {
    if (!isAdmin) return;
    const playerNames = Object.keys(teams).filter(name => !teams[name].relegated);
    if (playerNames.length % 2 !== 0) {
        showToast("Odd number of players – add a BYE or remove one.");
        return;
    }
    ...
    tournament.qualificationPlayoffs = tournament.qualificationPlayoffs.filter(p => p.round !== roundIndex);
    tournament.qualificationPlayoffs.push({ round: roundIndex, fixtures });
    saveToStorage();
    updateQualificationStandings();
    renderQualificationTable();
    renderPlayoffTabs();
    renderPlayoffFixtures();
    showToast(`Qualification Playoff ${roundIndex} created with ${fixtures.length} matches.`);
    addCommandHistory(`Created Playoff ${roundIndex} randomly.`);
}

function updateQualificationStandings() {
    for (let name in teams) {
        const p = teams[name];
        p.mp = 0; p.w = 0; p.d = 0; p.l = 0; p.gf = 0; p.ga = 0; p.gd = 0; p.pts = 0; p.formHistory = [];
        p.qualified = false;
    }
    tournament.qualificationPlayoffs.forEach(playoff => {
        playoff.fixtures.forEach(f => {
            if (f.played && !f.cancelled && teams[f.home] && teams[f.away]) {
    ...
    for (let name in teams) {
        const p = teams[name];
        p.pts = Math.max(0, p.pts - (p.deductedPoints || 0));
        p.gd = p.gf - p.ga;
    }
    const sorted = Object.values(teams).filter(t => !t.relegated).sort((a,b) => 
...
function renderQualificationTable() {
    ...
    sorted.forEach((p, idx) => {
        ...
        html += `<tr class="${rowClass} hover:bg-gray-50 transition cursor-pointer" onclick="showTeamDetails('${p.name}')">
            <td class="py-2 px-2 text-center font-bold text-xs">${pos}</td>
            <td class="py-2 px-2"><span class="font-semibold text-xs">${p.name}</span></td>
            <td class="py-2 px-1 text-center text-xs">${p.mp}</td>
            <td class="py-2 px-1 text-center text-emerald-600 text-xs">${p.w}</td>
            <td class="py-2 px-1 text-center text-xs">${p.d}</td>
            <td class="py-2 px-1 text-center text-rose-500 text-xs">${p.l}</td>
            <td class="py-2 px-1 text-center text-xs">${p.gf}</td>
            <td class="py-2 px-1 text-center text-xs">${p.ga}</td>
            <td class="py-2 px-1 text-center font-mono text-xs ${p.gd >= 0 ? 'text-emerald-600' : 'text-rose-500'}">${p.gd > 0 ? '+' + p.gd : p.gd}</td>
            <td class="py-2 px-2 text-center font-black text-indigo-600 text-xs">${p.pts}</td>
            <td class="py-2 px-2 text-center text-xs ${statusClass}">${statusText}</td>
        </tr>`;
    });
    ...
    if (statusSpan) {
        const totalPlayed = tournament.qualificationPlayoffs.reduce((sum, p) => sum + p.fixtures.filter(f => f.played).length, 0);
        const totalFixtures = tournament.qualificationPlayoffs.reduce((sum, p) => sum + p.fixtures.length, 0);
        const completed = totalPlayed === totalFixtures && totalFixtures > 0;
        if (completed) {
            statusSpan.innerText = `✅ ${TOURNAMENT_CONFIG.qualifiers} qualified • ${Object.keys(teams).length - TOURNAMENT_CONFIG.qualifiers} eliminated`;
            statusSpan.className = 'text-xs font-medium text-emerald-600';
        } else {
            statusSpan.innerText = `⏳ ${totalPlayed}/${totalFixtures} matches played`;
            statusSpan.className = 'text-xs font-medium text-amber-600';
        }
    }
}
...
function renderPlayoffTabs() {
    ...
    if (!container) return;
    const playoffs = tournament.qualificationPlayoffs;
    if (playoffs.length === 0) {
        container.innerHTML = '<span class="text-xs text-gray-400">No playoffs created yet.</span>';
        return;
    }
    container.innerHTML = '';
    playoffs.forEach((p, idx) => {
...
function renderPlayoffFixtures() {
    const container = document.getElementById('playoff-fixtures-container');
    if (!container) return;
...
function normalizeCommandText(cmd) {
    return cmd.toLowerCase().replace(/[^a-z0-9 ]/g, ' ').replace(/\s+/g, ' ').trim();
}

function detectIntent(normalized) {
    if (normalized.includes('create') || normalized.includes('generate') || normalized.includes('make')) return 'CREATE_STAGE';
    if (normalized.includes('qualify') || normalized.includes('top')) return 'QUALIFY';
    if (normalized.includes('show') || normalized.includes('view') || normalized.includes('display')) return 'VIEW';
    if (normalized.includes('reset') || normalized.includes('delete') || normalized.includes('wipe')) return 'RESET';
...
function detectStage(normalized) {
    ...
    if (normalized.includes('final') || normalized.includes('championship')) return { stage: 'FINAL', type: 'knockout' };
    return null;
}

function detectDrawType(normalized) {
    if (normalized.includes('random')) return 'RANDOM';
    if (normalized.includes('fixed') || normalized.includes('seeded')) return 'FIXED';
    return 'RANDOM';
}

function detectLegs(normalized) {
    if (normalized.includes('one leg') || normalized.includes('single leg') || normalized.includes('1 leg')) return 1;
    if (normalized.includes('two legs') || normalized.includes('2 legs') || normalized.includes('two leg')) return 2;
    return null;
}
...
function parseAdminCommand(command) {
    ...
    if (intent === 'QUALIFY') {
        return { intent: 'QUALIFY', stage: 'QUALIFICATION', source: 'ALL_PLAYERS' };
    }
    const stageObj = detectStage(normalized);
    if (!stageObj) return null;
    const drawType = detectDrawType(normalized);
    let legs = detectLegs(normalized);
    if (legs === null) {
        legs = (stageObj.stage === 'FINAL') ? 1 : 2;
    }
    const source = getSource(stageObj);
    return {
        intent: intent,
...
function previewCommand(parsed) {
    if (!parsed) return null;
    const stageLabel = getStageLabel(parsed.stage);
    let sourceLabel = '';
    if (parsed.source === 'ALL_PLAYERS') sourceLabel = 'All players';
    else if (parsed.source === 'QUALIFIED_TOP_16') sourceLabel = 'Top 16 qualified players';
    else if (parsed.source === 'PREVIOUS_STAGE_WINNERS') sourceLabel = 'Winners of previous stage';
    const legsLabel = parsed.legs === 2 ? 'Two legs' : 'One leg';
    let ties = 0, matches = 0;
    if (parsed.stageType === 'knockout') {
        const totalPlayers = (parsed.stage === 'ROUND_OF_16') ? 16 : (parsed.stage === 'QUARTER_FINAL') ? 8 : (parsed.stage === 'SEMI_FINAL') ? 4 : 2;
        ties = totalPlayers / 2;
        matches = ties * parsed.legs;
    }
    return {
        action: 'Create knockout stage',
        stage: stageLabel,
        source: sourceLabel,
        draw: parsed.drawType === 'RANDOM' ? 'Random draw' : 'Fixed draw',
        format: legsLabel,
        ties: ties,
        matches: matches
    };
...
function executeAdminCommandFromInput() {
    ...
    if (!parsed) { showToast("Command not understood. Try: 'Create Round of 16 randomly. Two legs.'"); return; }
    const preview = previewCommand(parsed);
    const previewDiv = document.getElementById('command-preview');
    if (preview && parsed.intent !== 'QUALIFY') {
        previewDiv.classList.remove('hidden');
        previewDiv.innerHTML = `
            <p class="font-bold text-indigo-700">COMMAND UNDERSTOOD</p>
            <div class="grid grid-cols-2 gap-1 text-xs">
                <span class="text-gray-500">Action:</span><span>${preview.action}</span>
                <span class="text-gray-500">Stage:</span><span>${preview.stage}</span>
                <span class="text-gray-500">Source:</span><span>${preview.source}</span>
                <span class="text-gray-500">Draw:</span><span>${preview.draw}</span>
                <span class="text-gray-500">Format:</span><span>${preview.format}</span>
                <span class="text-gray-500">Ties:</span><span>${preview.ties}</span>
                <span class="text-gray-500">Matches:</span><span>${preview.matches}</span>
            </div>
            <div class="flex gap-3 mt-3">
                <button onclick="confirmCommand('${command}')" class="bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-1.5 rounded-lg text-sm font-bold">✅ Generate</button>
                <button onclick="document.getElementById('command-preview').classList.add('hidden')" class="bg-gray-200 hover:bg-gray-300 px-4 py-1.5 rounded-lg text-sm">Cancel</button>
            </div>
        `;
    } else {
...
function confirmCommand(command) {
    document.getElementById('command-preview').classList.add('hidden');
    const parsed = parseAdminCommand(command);
    if (!parsed) return;
    executeParsedCommand(parsed, command);
}
...
function addCommandHistory(text) {
    ...
    tournament.commandHistory.push(entry);
    if (tournament.commandHistory.length > 50) tournament.commandHistory.shift();
    saveToStorage();
...
function renderCommandHistory() {
    ...
    if (!container) return;
    const history = tournament.commandHistory;
    if (history.length === 0) {
        container.innerHTML = '<div class="text-gray-400 text-xs">No commands yet.</div>';
        return;
    }
    container.innerHTML = history.slice().reverse().map(entry => 
        `<div class="flex justify-between border-b border-gray-100 py-1"><span class="text-gray-700">${escapeHtml(entry.text)}</span><span class="text-gray-400 text-[10px]">${new Date(entry.timestamp).toLocaleString()}</span></div>`
    ).join('');
}

function toggleCommandHistory() {
    const list = document.getElementById('command-history-list');
    if (list) list.classList.toggle('hidden');
}

function quickCommand(text) {
    document.getElementById('command-input').value = text;
    executeAdminCommandFromInput();
}
...
function createKnockoutStage(stageId, source, drawType, legs) {
    ...
    renderKnockoutBracket();
    updateTournamentStatusBar();
    showToast(`${getStageLabel(stageId)} created with ${ties.length} ties.`);
}
...
function calculateAggregate(tie) {
    const legs = tie.legs;
...
function updateTournamentStatusBar() {
                ...
                <div class="p-4 bg-gradient-to-r from-indigo-600 to-purple-600 text-white sticky top-0 z-10">
                    <div class="flex justify-between items-center">
                        <div>
                            ...
                            <p class="text-xs text-white/80">Rounds 1 - ${halfRounds} | ${CURRENT_LEAGUE === 'championsleague' ? 'Premier League' : 'Championship'}</p>
...
function resetTournament() { 
    if (confirm("Wipe ALL data for Champions League? Cannot be undone.")) 
