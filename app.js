...
function initRealtimeDatabaseSync() {
    ...
    // Live alerts: listen to fixture changes
...
// ==================== TICKER (unchanged) ====================
function updateTickerFacts() { /* keep your existing code */ }
function generateTickerFacts() { /* keep your existing code */ }

// ==================== ADMIN MODE (unchanged) ====================
function handleAdminToggleClick() { /* keep existing */ }
function closePasswordModal() { /* keep existing */ }
function verifyAdminPassword() { /* keep existing */ }
...
function updateAdminUIElements() { /* keep existing, calls renderTable, renderGameweekTabs, renderFixtures */ }

// ==================== PASSWORD CHANGE (unchanged) ====================
function openChangePasswordModal() { /* keep existing */ }
function closeChangePasswordModal() { /* keep existing */ }
function updateMasterPassword() { /* keep existing */ }
...
// ==================== TOURNAMENT SETUP (extended with events/report fields) ====================
function generateTeamInputs() { /* keep existing */ }
    ...
    for (let r = 0; r < rounds; r++) {
        for (let m = 0; m < n / 2; m++) {
            ...
            if (list[homeIdx].name !== "BYE" && list[awayIdx].name !== "BYE") {
                fixtures.push({ 
                    id: fixtures.length, round: r + 1, home: list[homeIdx].name, away: list[awayIdx].name,
                    homeScore: null, awayScore: null, played: false,
                    comment: null, predictions: [], banter: [],
                    events: [], report: null  // NEW fields
                });
            }
        }
    }
...
function shuffleRound(roundNumber) { /* keep existing */ }
function swapFixture(fixtureId) { /* keep existing */ }
function editFixtureTeamName(fixtureId, side) { /* keep existing */ }
function closeTeamSelectModal() { /* keep existing */ }
function confirmTeamSelection() { /* keep existing */ }

// ==================== STANDINGS & CHAMPION DETECTION (#2) ====================
...
function checkAndCelebrateChampion() {
    ...
    const allPlayed = fixtures.filter(f => !teams[f.home]?.relegated && !teams[f.away]?.relegated).length > 0 &&
                      fixtures.every(f => f.played || teams[f.home]?.relegated || teams[f.away]?.relegated);
    if (!allPlayed) return;
    const sorted = activeTeams.sort((a,b) => b.pts - a.pts || b.gd - a.gd || b.gf - a.gf);
    if (sorted.length === 0) return;
    const champion = sorted[0];
    db.ref('tournament_data/champion').set({ name: champion.name, date: new Date().toISOString() });
    if (typeof confetti === 'function') confetti({ particleCount: 200, spread: 70, origin: { y: 0.6 } });
    showToast(`🏆 ${champion.name} are champions! 🏆`);
}
...
function renderTable() {
    ...
    const allPlayed = fixtures.filter(f => !teams[f.home]?.relegated && !teams[f.away]?.relegated).length > 0 &&
                      fixtures.every(f => f.played || teams[f.home]?.relegated || teams[f.away]?.relegated);
    const championName = allPlayed ? sorted[0]?.name : null;
    ...
    sorted.forEach((team, idx) => {
        ...
        const trophyIcon = (championName === team.name && allPlayed) ? `<span class="ml-1 text-yellow-500 text-xs">🏆</span>` : "";
...
function renderFixtures() {
    ...
    fixtures.filter(f => f.round === currentSelectedRound && !teams[f.home]?.relegated && !teams[f.away]?.relegated).forEach(f => {
        ...
        if (isAdmin) {
            ...
            container.innerHTML += `<div class="bg-gray-50/60 p-3 rounded-xl border border-gray-100 shadow-sm w-full fixture-card" data-fixture-id="${f.id}"><div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div class="flex-1 text-right ${played && f.homeScore > f.awayScore ? 'text-gray-900 font-bold' : 'text-gray-600'}">${f.home}</div><div class="flex justify-center">${played ? `<div class="bg-gray-100 px-3 py-1 rounded-full font-mono font-bold text-sm">${f.homeScore} - ${f.awayScore}</div>` : `<button onclick="openPredictionsModal(${f.id})" class="text-[11px] bg-gray-100 hover:bg-indigo-50 px-3 py-1 rounded-full">🔮 Predictions</button>`}</div><div class="flex-1 text-left ${played && f.awayScore > f.homeScore ? 'text-gray-900 font-bold' : 'text-gray-600'}">${f.away}</div></div><div class="mt-2 flex justify-center gap-1"><button onclick="showMatchComment(${f.id})" class="text-[11px] bg-gray-100 hover:bg-gray-200 px-3 py-1 rounded-full">📖</button><button onclick="openBanterModal(${f.id})" class="text-[11px] bg-purple-50 hover:bg-purple-100 px-3 py-1 rounded-full">🤣 Banter</button></div></div>`;
        }
    });
    initSortable();
}

// SortableJS initialization (#13)
function initSortable() {
    ...
    currentSortable = new Sortable(container, {
        ...
        onEnd: function() {
            const newOrder = [];
            document.querySelectorAll('#fixtures-container .fixture-card').forEach(card => {
                newOrder.push(parseInt(card.dataset.fixtureId));
            });
            ...
            let idx = 0;
            for (let f of orderedFixtures) {
                const globalIndex = fixtures.findIndex(x => x.id === f.id);
                fixtures[globalIndex] = f;
            }
            saveToStorage();
            renderFixtures();
        }
...
// ==================== TEAM DETAILS WITH ADVANCED STATS (#5) ====================
...
// ==================== SAVE RESULT (with events & report) ====================
...
// ==================== RELEGATION ZONE (unchanged) ====================
function relegateTeam(teamName) { /* keep existing */ }
function restoreTeam(teamName) { /* keep existing */ }
function renderRelegatedTeams() { /* keep existing */ }

// ==================== PREDICTIONS (unchanged) ====================
function openPredictionsModal(fixtureId) { /* keep existing */ }
function closePredictionsModal() { /* keep existing */ }
function renderPredictions(fixtureId) { /* keep existing */ }
function submitPrediction() { /* keep existing */ }
function deletePrediction(fixtureId, index) { /* keep existing */ }

// ==================== BANTER (unchanged) ====================
function openBanterModal(fixtureId) { /* keep existing */ }
function closeBanterModal() { /* keep existing */ }
function renderBanterMessages(fixtureId) { /* keep existing */ }
function postBanter() { /* keep existing */ }
function deleteBanter(fixtureId, index) { /* keep existing */ }
...
// Expose all functions (keep existing, plus new ones)
