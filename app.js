...
function initGroupInfoListener() {
    ...
    groupInfoRef.on('value', (snapshot) => {
        ...
        document.getElementById('group-description-display').innerText = info.description || 'Official league chat';
...
function initializeTournament() {
    ...
    list.forEach(item => {
        if (item.name !== "BYE") teams[item.name] = {
            name: item.name, mp: 0, w: 0, d: 0, l: 0, gf: 0, ga: 0,
            gd: 0, pts: 0, deductedPoints: 0, formHistory: [], relegated: false
        };
    });
    ...
    rounds.forEach((roundFixtures, roundIndex) => {
        roundFixtures.forEach(({ home, away }) => {
            fixtures.push({
                ...
                comment: null, predictions: [], banter: [], events: [],
                report: null, deadline: null
            });
        });
    });
    ...
    // NEW: immediately load the tournament data after saving
    db.ref('tournament_data').once('value', (snapshot) => {
        const data = snapshot.val();
        if (data) {
            loadTournamentData(data);
            ...
            document.getElementById('dashboard-section')?.classList.remove('hidden');
            document.getElementById('deadline-clock')?.classList.remove('hidden');
            showToast(`Tournament launched with ${Object.keys(teams).length} teams!`);
        } else {
            showToast("Error loading tournament data. Please refresh.");
        }
    }).catch(err => showToast("Error: " + err.message));
}
...
// ==================== TEAM REPLACEMENT (ADMIN ONLY) ====================
function replaceTeam(oldTeamName, newTeamName) {
    if (!isAdmin) return showToast("Admin only");
    if (!teams[oldTeamName]) return showToast(`Team "${oldTeamName}" not found`);
    if (teams[newTeamName]) {
        const confirm = window.confirm(`Team "${newTeamName}" already exists. Replace anyway? Stats will be merged?`);
        if (!confirm) return;
        // Optional: merge stats? I'll just warn and not merge.
    }
    
    // 1. Create new team object with zero stats
    teams[newTeamName] = {
        name: newTeamName,
    ...
    // 2. Update future fixtures (not played yet)
    fixtures.forEach(f => {
        if (!f.played && !f.cancelled) {
            if (f.home === oldTeamName) f.home = newTeamName;
            if (f.away === oldTeamName) f.away = newTeamName;
        }
    });
    
    // 3. Update knockout matches (future legs)
    knockoutMatches.forEach(k => {
        if (!k.played && !k.cancelled) {
            if (k.home === oldTeamName) k.home = newTeamName;
            if (k.away === oldTeamName) k.away = newTeamName;
        }
    });
    
    // 4. Remove old team from `teams` object
    delete teams[oldTeamName];
    
    // 5. Recalculate league table (stats will be recomputed from played matches only)
    updateTableCalculations();
    renderTable();
    renderGameweekTabs();
    renderFixtures();
    ...
    generateTickerFacts();
    saveToStorage();
    showToast(`✅ Team "${oldTeamName}" replaced by "${newTeamName}". Past results preserved.`);
}

function promptReplaceTeam(oldTeamName) {
    const newTeamName = prompt(`Enter new team name to replace "${oldTeamName}":`);
    if (newTeamName && newTeamName.trim()) {
        replaceTeam(oldTeamName, newTeamName.trim());
    }
}
...
function renderTable() {
    ...
    <button onclick="event.stopPropagation(); promptReplaceTeam('${team.name}')" class="text-[9px] bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded-full hover:bg-blue-100">🔄 Replace</button>
</td>` : "";
    ...
    if (phaseIndicator) phaseIndicator.innerText = tournamentPhase === 'league' ? '🏆 League' : '🥇 Knockout Stage';
    generateTickerFacts();
}
